...
>(({ className, sideOffset = 6, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={cn(
        "z-50 min-w-[10rem] overflow-hidden rounded-lg border border-ink-200 bg-white p-1.5 text-ink-900 shadow-elev-lg",
...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Label
    ref={ref}
    className={cn(
      "px-2.5 pb-1 pt-2 font-mono text-[10px] uppercase tracking-[0.14em] text-ink-500",
      inset && "pl-8",
      className,
    )}
    {...props}
...
const DropdownMenuShortcut = ({
  ...
  return (
    <span
      className={cn(
        "ml-auto font-mono text-[10px] tracking-widest text-ink-400",
        className,
      )}
      {...props}
    />
