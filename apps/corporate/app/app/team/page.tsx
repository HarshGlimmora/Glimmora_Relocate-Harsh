...
const roleMeta: Record<string, { Icon: typeof Crown; label: string; cls: string }> = {
  ...
  LEAD:     { Icon: Shield, label: "Mobility lead",  cls: "bg-moss-50 border-moss-200 text-moss-800" },
  MANAGER:  { Icon: Users,  label: "Manager",  cls: "bg-ink-50 border-ink-200 text-ink-800" },
...
export default async function TeamPage() {
  ...
  const members = await prisma.corporateMembership.findMany({
    ...
    <div className="mx-auto max-w-[1200px] px-6 py-10 md:px-10 md:py-12">
      <header className="mb-10 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-3 font-sans text-[clamp(2.25rem,4.5vw,3.5rem)] font-semibold leading-[1.02] tracking-[-0.03em] text-ink-900">
            Who runs {organization.name}'s program.
          </h1>
          <p className="mt-4 max-w-xl text-[15.5px] leading-[1.6] text-ink-600">
            Invite mobility leads, finance partners, or viewers. Each gets scoped permissions.
          </p>
        </div>
        <button
          disabled
      ...
      <section className="mb-10 rounded-2xl border border-ink-200 bg-white overflow-hidden">
        <ul className="divide-y divide-ink-100">
          {members.map((m) => {
            const role = roleMeta[m.role] ?? roleMeta.MANAGER;
            const RIcon = role.Icon;
            return (
              <li key={m.id} className="flex items-center gap-4 px-5 py-4 md:gap-6 md:px-6">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-moss-600 text-[13px] font-semibold text-white">
            ...
            return (
              <div key={k} className="rounded-2xl border border-ink-200 bg-white p-5">
                ...
                <p className="mt-3 text-[13px] text-ink-600">
                  {k === "OWNER" && "Billing, master contract, full program authority."}
                  {k === "LEAD" && "Policy CRUD, approvals, pipeline oversight. Day-to-day program ops."}
                  {k === "MANAGER" && "Raise exception requests, see their team's relocations."}
                  {k === "FINANCE" && "Invoices, spend reports. No employee PII."}
                  {k === "VIEWER" && "Read-only access to reports."}
                </p>
              </div>
            );
          })}
