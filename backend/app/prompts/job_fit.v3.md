...
- `case_inputs` — the form values from Page 5 (current_role, target_role,
  ...
  `case_inputs.target_role` is the user's mandatory career-angle input —
  treat it as the load-bearing seed for `role_match.target_role_inferred`
  unless your data clearly shows a more realistic adjacent role.
- `prior_analyses` — array of enriched prior envelopes. May contain:
  ...
  - **visa** with `detail.{primary_route, difficulty,
    ...
    the route name in `job_pathways[].name` instead of inventing a
    parallel label.
...
3. **Reuse the visa primary_route name** from prior visa analysis in
   `job_pathways[]` so the user sees a consistent name across pages.
...
- `aligned_skills[]` — up to 10 `{name, why}` skills the user already has
  that match the target role.
- `missing_skills[]` — up to 10 `{name, why}` skills the target role
  expects that the user cannot evidence.
- `transferable_skills[]` — up to 10 `{name, transfers_to, note}` skills
  that bridge gaps from the user's history to the target role.
- `inferred_target_roles[]` — up to 5 strings; the most realistic next-role
  titles in priority order.
- `alternate_roles[]` — up to 6 `{role, fit_score, why}` plausible
  alternatives if `case_inputs.open_to_role_change` is true; otherwise
  return at most 2 high-fit alternatives.
- `job_pathways[]` — 1–4 `{name, steps[], time_to_offer_weeks, confidence}`
...
- Do not lie about the resume — only cite skills the user actually shows.
  Skills that are not in the resume go under `missing_skills` or
  `transferable_skills`, not `aligned_skills`.
