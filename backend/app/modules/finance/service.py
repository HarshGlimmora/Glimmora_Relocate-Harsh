...
from app.modules.finance.schemas import FinanceDetail, FinanceInputs
...
KIND = AnalysisKind.FINANCE.value
    ...
    async def run(
        ...
        *,
        ...
        if not merged_inputs.get("target_country"):
            raise BadRequest(
                "financial-feasibility requires target_country on the profile or in the request body."
            )
        if (
            merged_inputs.get("expected_salary") in (None, 0)
            and merged_inputs.get("current_salary") in (None, 0)
        ):
            raise BadRequest(
                "financial-feasibility requires expected_salary or current_salary."
            )
        ...
        if not body.force:
            ...
            if cached is not None and cached.status == "ready":
                log_event(
                    ...
                    case_id=case_id,
                    user_id=user_id,
                    module_name="financial-feasibility",
...
def _build_inputs(
    *, profile: UserProfile, case_inputs: dict, body: FinanceInputs
) -> dict[str, Any]:
    body_d = body.model_dump(exclude_unset=True)

    out = {
        "current_salary": _first_present(
            body_d.get("current_salary"),
            case_inputs.get("current_salary"),
            profile.current_salary,
        ),
        "expected_salary": _first_present(
            body_d.get("expected_salary"),
            case_inputs.get("expected_salary"),
            profile.expected_salary,
        ),
        "current_currency": (
            body_d.get("current_currency")
            or case_inputs.get("current_currency")
        ),
        "salary_currency": (
        ...
        "monthly_budget": _first_present(
            body_d.get("monthly_budget"), case_inputs.get("monthly_budget")
        ),
        "savings": _first_present(
            body_d.get("savings"), case_inputs.get("savings")
        ),
        "family_size": _first_present(
            body_d.get("family_size"), case_inputs.get("family_size"), 1
        ),
        "rent_expectation": _first_present(
            body_d.get("rent_expectation"), case_inputs.get("rent_expectation")
        ),
        "cost_sensitivity": (
            body_d.get("cost_sensitivity")
            or case_inputs.get("cost_sensitivity")
            or "medium"
        ),
    ...
    for k in ("target_country",):
        if out.get(k):
            out[k] = out[k].upper()
    for k in ("current_currency", "salary_currency"):
        if out.get(k):
            out[k] = out[k].upper()
    return out
...
def _assemble_envelope(
    *,
    ...
    full = AnalysisEnvelope[FinanceDetail](
