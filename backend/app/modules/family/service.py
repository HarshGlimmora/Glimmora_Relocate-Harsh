...
Same pipeline shape as the prior analysis modules. Adds two pieces of
behaviour specific to family:

  - Solo vs with-family mode is computed from the body + case snapshot
    BEFORE the AI call, and is also pinned in the prompt context so the
    model cannot accidentally pick the wrong mode.
  - Requires `target_country` to generate (every recommendation depends on
    destination context).
"""
...
from app.modules.family.schemas import FamilyImpactDetail, FamilyInputs
...
KIND = AnalysisKind.FAMILY.value
    ...
    async def run(
        ...
        *,
        ...
        if not merged_inputs.get("target_country"):
            raise BadRequest(
                "family-relocation requires target_country on the profile or in the request body."
            )
...
def _build_inputs(
    *, profile: UserProfile, case_inputs: dict, body: FamilyInputs
) -> dict[str, Any]:
    body_d = body.model_dump(exclude_unset=True)

    spouse = body_d.get("spouse")
    if spouse is None:
        spouse = case_inputs.get("spouse") or {"moving": False, "has_career": False}

    children = body_d.get("children")
    if children is None:
        children = case_inputs.get("children") or []

    parents = body_d.get("parents")
    if parents is None:
        parents = case_inputs.get("parents") or {
            "moving": False,
            "dependency_level": "none",
            "healthcare_sensitivity": "low",
        }

    moving_with_family = body_d.get("moving_with_family")
    if moving_with_family is None:
        moving_with_family = case_inputs.get("moving_with_family")
    if moving_with_family is None:
        # Derive: if anyone in spouse/children/parents is moving, treat as family.
        moving_with_family = bool(
            (spouse or {}).get("moving")
            or any((children or []))
            or (parents or {}).get("moving")
        )

    out = {
        "moving_with_family": bool(moving_with_family),
        "mode_hint": "with_family" if moving_with_family else "solo",
        "spouse": spouse,
        "children": children,
        "parents": parents,
        "housing_requirement": body_d.get("housing_requirement")
        or case_inputs.get("housing_requirement"),
        "family_budget_impact": body_d.get("family_budget_impact")
        or case_inputs.get("family_budget_impact"),
        # Destination + origin context for the model
...
def _assemble_envelope(
    *,
    ...
    full = AnalysisEnvelope[FamilyImpactDetail](
