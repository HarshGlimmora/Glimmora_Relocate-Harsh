"""Culture & Language service.

Same pipeline shape. Culture is reasoning-light + qualitative — uses the
FAST tier (`gemini_model`).
"""
...
from app.modules.culture.repository import CultureRepository
...
KIND = AnalysisKind.CULTURE.value
...
class CultureService:
    ...
    def __init__(
        ...
        *,
        ...
        self.repo = CultureRepository(session)
    ...
    async def run(
        ...
        *,
        ...
        if not merged_inputs.get("target_country"):
            raise BadRequest(
                "culture-language requires target_country on the profile."
            )
        ...
        # Culture is qualitative — Flash tier is fine.
...
def _assemble_envelope(
    *,
    ...
    full = AnalysisEnvelope[CultureDetail](
