...
Same pipeline shape. Documents flagged Flash-tier in the plan (it's
enumeration, not reasoning), but the gateway resolves the model from the
ModelTier hint — we use FAST here to honour that.
"""
...
KIND = AnalysisKind.DOCUMENTS.value
    ...
    async def run(
        ...
        *,
        ...
        if not merged_inputs.get("target_country"):
            raise BadRequest(
                "document-checklist requires target_country on the profile or in the request body."
            )
        ...
        if not body.force:
            ...
            if cached is not None and cached.status == "ready":
                log_event(
                    ...
                    case_id=case_id,
                    user_id=user_id,
                    module_name="document-checklist",
        ...
        # Documents is enumeration, not reasoning — use the cheap tier.
...
def _build_inputs(
    *, profile: UserProfile, case_inputs: dict, body: DocumentsInputs
) -> dict[str, Any]:
    body_d = body.model_dump(exclude_unset=True, mode="json")

    body_status = body_d.get("current_document_status")
    if body_status is not None:
        document_status = body_status
    else:
        document_status = (
            case_inputs.get("current_document_status")
            or profile.current_document_status
            or {}
        )
...
def _assemble_envelope(
    *,
    ...
    full = AnalysisEnvelope[DocumentChecklistDetail](
